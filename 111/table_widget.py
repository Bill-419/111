...
class TableWidget(QTableWidget):
    ...
    def find_record_by_title(self, title):
        # 在表格中查找与问题单号匹配的记录
        for row in range(self.rowCount()):
            issue_number_item = self.item(row, 0)
            if issue_number_item and issue_number_item.text() == title:
                record = issue_number_item.data(Qt.UserRole)
                return record
        return None